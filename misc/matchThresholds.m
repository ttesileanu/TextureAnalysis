...
for i = 1:length(target_groups)
    ...
    mask = strcmp(groups, crt_group) & ...
        cellfun(@(v) norm(v - crt_direction) < tol, directions);
    if sum(mask) == 0
        error([mfilename ':nomatch'], ['No match found for group ' crt_group ', direction ' num2str(crt_direction) '.']);
    end
    if sum(mask) > 1
        error([mfilename ':multimatch'], ['Multiple matches found for group ' crt_group ', direction ' num2str(crt_direction) '.']);
    end
    target_thresholds(i) = thresholds(mask);
end
