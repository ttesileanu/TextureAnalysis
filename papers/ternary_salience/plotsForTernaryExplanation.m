...
%% Draw example planes with simplex and patches

% keep things reproducible
rng(3434);

% planes to show
planes = {'AB_1_1', 'AD_1_2'};
...
for i = 1:length(planes)
    ...
    for k = 1:length(texAxes)
        ...
        crtT = barPositions{k};
        insetbar([crtT patchDim patchDim/2], crtLoc + 0.05, 'color', barColor);
    end
    
    % draw a bar plot in the center
    insetbar([centerBarPosition patchDim patchDim/2], [1/3 1/3 1/3] + 0.05, ...
        'color', barColor);
    
    showTernaryTexturesSingle(plane, 'patchExtent', patchDim*0.91, 'offset', 0.3, ...
        'centerPatch', true, 'patchSize', 32);
    ...
    safePrint(fullfile('figs', 'draft', ['exampleTernaryPlane_' plane]));
end
...
% draw all the points
smartscatter(secondOrder, fourthOrder, 'density', false, 'color', [0.8 0.8 0.8], ...
    'alpha', 0.05, 'maxpoints', inf, 'size', 5);
% overlay the in-focus points
% [0    0.4470    0.7410]
smartscatter(secondOrderFocus, fourthOrderFocus, 'density', false, 'color', [0 0 0], ...
    'alpha', 0.05, 'maxpoints', inf, 'size', 5);
% xlabel(group1);
% ylabel(group2);
...
% beautify and save
beautifygraph('fontscale', 0.6667, 'ticksize', 11, 'minorticks', 'off');
preparegraph;
