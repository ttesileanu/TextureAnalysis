function [imgOut, crop] = equalize(img, varargin)
% EQUALIZE Perform histogram equalization in an image, potentially
% patch-by-patch.
%   imgOut = EQUALIZE(img) performs histogram equalization on the image by
%   mapping all intensity values to the interval [0, 1], and ensuring that
...
%   deactivate this).
%
%   imgOut = EQUALIZE(img, patchSize) splits the image into non-overlapping
%   patches of size `patchSize` and then equalizes within each patch.
%   `patchSize` can also be a pair of integers to use non-square patches.
%   In this case, the order is row (y) size first, and then column (x)
%   size, `[patchSizeY, patchSizeX]`! Anything left over on the right and
%   bottom edges of the image if `size(img)` is not an integer multiple of
%   `patchSize` is trimmed.
%
%   imgOut = EQUALIZE(img, patchSize, 'perpixel', true) calculates the
...
%
%   [imgOut, crop] = EQUALIZE(...) also returns a vector of four elements
%   [row1, col1, row2, col2] identifying the range in the original image
%   corresponding to the equalized image.
%
%   EQUALIZE(..., 'jitter', amount) sets the amount of jitter added to the
%   image to distinguish pixels with exactly equal values. The jitter is
%   added as uniform noise in the interval [-amount, amount]. Set this to 0
%   to have no jitter. The default value is equal to
%   `eps(max(abs(img(:))))`.
