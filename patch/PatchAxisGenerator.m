classdef PatchAxisGenerator < handle
    ...
    %   in the texture axis. The constructor automatically normalizes the
    %   texture axis, setting the sum to 1 before texture generation.
    %
    ...
    %   parameter. Note that mixed-axis texture generation typically
    %   requires some Monte Carlo steps. The number of iterations to use
    %   can be controlled using the `mcIterations` property.
    %
...
%             end
        ...
        function init(obj)
            ...
            if ~isempty(obj.locations)
                ...
                % find the maximum axis location we can go to
                if isempty(obj.maxLocation)
                    % explict formula in the single-group case
        ...
        function ok = next(obj)
            ...
            % increment current location, unless we're done
        ...
        function res = samples(obj, n)
            ...
            %   obj.SAMPLES generates one patch.
            %   obj.SAMPLES(n) generates 'n' patches at the same location.
            %       These are returned in a 3d array.
            if nargin < 2
                n = 1;
            end
            ...
            % we can't generate samples here
