...
discrScalings = arrayfun(@(i) norm(stats.reps.lda.discriminant(:, i)), 1:nreps);
discrNorm = bsxfun(@rdivide, stats.reps.lda.discriminant, discrScalings);
% discrProj = discrNorm*stats.summary.both.allPC;
discrBbox = zeros(4, 1); % [minX, maxX, minY, maxY]
for i = 1:nreps
    threshDist = -stats.reps.lda.const(i)/discrScalings(i);
    discrPoints = [...
        (threshDist - 0.5)*discrNorm(:, i)' ; ...
        threshDist*discrNorm(:, i)' ; ...
        (threshDist + 0.5)*discrNorm(:, i)'];
    discrPointsProj = discrPoints*stats.summary.both.allPC;
    
    plot(discrPointsProj(1:2, 1), discrPointsProj(1:2, 2), '-o', 'color', [1 0 0]);
    plot(discrPointsProj(2:3, 1), discrPointsProj(2:3, 2), ':o', 'color', [1 0.6 0]);
    
    discrBbox(1) = min([discrBbox(1) ; discrPointsProj(:, 1)]);
    discrBbox(2) = max([discrBbox(1) ; discrPointsProj(:, 1)]);
    discrBbox(3) = min([discrBbox(2) ; discrPointsProj(:, 2)]);
    discrBbox(4) = max([discrBbox(2) ; discrPointsProj(:, 2)]);
    
%     plot(threshPt(1), threshPt(2), '.', 'color', [1 0 0]);
%     plot(threshPt(1), threshPt(2), '.', 'color', [1 0.6 0]);
%     line([threshPt(1), threshPt(1)-discrProj(i, 1)], ...
%         [threshPt(2), threshPt(2)-discrProj(i, 2)], 'color', [1 0 0]);
%     line([threshPt(1), threshPt(1)+discrProj(i, 1)], ...
%         [threshPt(2), threshPt(2)+discrProj(i, 2)], 'linestyle', ':', 'color', [1 0.6 0]);
end
...
txt1Pos = 1.2*discrBbox([1 3]);
txt2Pos = 1.2*discrBbox([2 4]);
