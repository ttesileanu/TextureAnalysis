% Analyzing three grayscale level-data using stochastic binary texture
% statistics.
...
for i = 1:length(tex_axes)
    ...
    for j = 1:length(crt_dot_locs)
        ...
        for k = 1:n_per_dot
            ...
            [~, crt_cont_stats(k, :)] = processBlock(...
                stochasticBinarize(crt_patch), 2);
        end
...
save(fullfile('save', 'three_psycho_via_stochbin_extended.mat'), ...
...
load(fullfile('save', 'three_psycho_via_stochbin_extended.mat'));

%% Load natural image stats

natural_stats0 = open(fullfile('save', 'natural_nosky_stochastic_binary_with_focus.mat'));
res_choice = 4;
...
safe_print(fullfile('figs', 'G3vsStochBin', ['thresholds_in_10d' save_tag]), 'png');
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_pred_thresholds' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_pred_thresholds_two_fits' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_ellipses' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['noiserad_dependence' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['noiserad_dependence_low_order' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_ellipses_low_order' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_vs_g3ni' save_tag]));
...
safe_print(fullfile('figs', 'G3vsStochBin', ['cont_vs_g3_residuals' save_tag]));
