% Analyzing three grayscale level-data using vanilla binary texture
% statistics.
    ...
    for j = 1:length(crt_dot_locs)
        ...
        for k = 1:n_per_dot
            ...
            [~, crt_cont_stats(k, :)] = processBlock(...
                quantize(equalize(crt_patch), 2), 2);
        end
...
save(fullfile('save', 'three_psycho_via_bin_extended.mat'), ...
...
load(fullfile('save', 'three_psycho_via_bin_extended.mat'));

%% Load natural image stats

natural_stats0 = open(fullfile('save', 'natural_nosky_binary_with_focus.mat'));
res_choice = 4;
...
safe_print(fullfile('figs', 'G3vsBin', ['thresholds_in_10d' save_tag]), 'png');
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_pred_thresholds' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_pred_thresholds_two_fits' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_ellipses' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['noiserad_dependence' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['noiserad_dependence_low_order' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_ellipses_low_order' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_vs_g3ni' save_tag]));
...
safe_print(fullfile('figs', 'G3vsBin', ['cont_vs_g3_residuals' save_tag]));
