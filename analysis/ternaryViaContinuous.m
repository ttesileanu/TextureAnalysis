...
for i = 1:length(sub_edirs)
    ...
    for j = 1:length(subjects)
        ...
        stdev_ratios_by_subject{j} = [stdev_ratios_by_subject{j} crt_stdev_ratios_subj(:)'];
    end
...
axis_scaling = cellfun(@(crt_axis) sqrt(3*norm(crt_axis)^2 - 1)/sqrt(2), tex_axes);

hold on;
masked_predicted_thresholds = axis_scaling(mask) .* predicted_thresholds(mask);
...
h = errorbar(axis_scaling(mask) .* predicted_thresholds(mask), thresholds(mask), ...
    thresholds_std_neg(mask), thresholds_std_pos(mask), ...
    'marker', 'none', 'color', [0.5 0.5 0.5], 'linestyle', 'none');
h.CapSize = 0;
th_h = smartscatter(axis_scaling(mask) .* predicted_thresholds(mask), thresholds(mask), 'color', threshold_colors, ...
    'density', false);
...
%     'corrtype', 'spearman');
drawfitline(axis_scaling(mask) .* predicted_thresholds(mask), thresholds(mask), 'line', [1 0], ...
...
for i = 1:length(thresholds)
    ...
    text(axis_scaling(i) * predicted_thresholds(i)+0.01, thresholds(i), ...
        groups{i}, ...
        'fontsize', 6);        
...
safe_print(fullfile('figs', 'g3_via_cont_pred_means'), 'png');
...
%% Make plots for subset of texture groups

fig = figure;
fig.Units = 'inches';
fig.Position = [1 1 11 4];

unique_groups = {'AB_1_1', 'AB_1_2'};
...
%     py = 0;
    px = i-1;
    ax.Units = 'normalized';
    ax.OuterPosition = [px/length(unique_groups), 0, 1/length(unique_groups), 1];
    ...
    for j = 1:length(mask_idxs)
        ...
        % plot
        h_meas = [h_meas plot(crt_t1, crt_t2, 'kx', 'linewidth', 1)]; %#ok<AGROW>
        h_pred = [h_pred plot(crt_pred_t1, crt_pred_t2, 'rx', 'linewidth', 1)]; %#ok<AGROW>
    end
...
safe_print(fullfile('figs', 'g3_via_cont_match_mismatch'));
