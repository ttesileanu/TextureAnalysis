...
%   `noise_radius2` in continuous-texture coordinates, after they are
...
%   having 10 columns and an arbitrary number of rows, depending on how
%   many patches are generated at each G=3 location. The locations along the
%   G=3 axis are those identified by the vector cell array `map_locs`, with
%   `map_locs{i}` corresponding to `stats_mapping{i}`.
%
...
for i = 1:n
    found_loc = [];
    found_thresh = [];
    crt_dot_locs = map_locs{i};
    crt_stats = stats_mapping{i};
